...
void ofApp::setup(){
	...
	loadAddress();
	reciever1.setup(recievePort);
	sender1.setup(ipAddress, sendPort);
...
void ofApp::loadAddress() {
	ofXml xml;
	xml.load("Address.xml");
	xml.setTo("address");
	ipAddress = xml.getValue("ip","127.0.0.1");
	ofLog() << "Loaded address: " << ipAddress;

	sendPort = xml.getIntValue("sPort");
	recievePort = xml.getIntValue("rPort");
}
